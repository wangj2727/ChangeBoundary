                        ...
                        wellPanel(
                          ...
                          fluidRow(
                            column(3,textInput(inputId = "planned_times",
                                                  label = "Enter the original future times after adaptation:",
                                                  value="1"),
  ...
  tabPanel("R Codes",
         fluidRow(
           column(10, offset=1,
                     ...
                     h5("R version 4.1.3 (2022-03-10)"),
                     h5("Key package version:"),
                     HTML("<ul><li>ldbounds_2.0.0</li><li>shiny_1.7.1</li><li>dplyr_1.1.0</li><li>shinycssloaders_1.0.0</li><li>shinyjs_2.1.0</li><li>bslib_0.3.1</li></ul>")
           )
           ))
))
...
server <- function(input, output, session) {
  ...
  output$figure1 <- renderImage({
    list(
      src = file.path("7stepproc3.png"),  ### might want to update it 
  ...
  observeEvent(input$clickthis, {
      ...
      times <- unique(c(times_actual(), times_planned()[times_planned()>max(times_actual())]))
      times_update <- unique(c(times_actual(), newfutimes()[newfutimes()>max(times_actual())]))
      ...
      futimes<-times_planned() # future times, orig.
      fubounds <- boundary[unlist(lapply(times_planned(), function(x){which(times==x)}))] # orig future bound.
  ...
  ##########################################
    ...
    output$figure4 <- renderPlot({
      ...
      isolate({
        ...
        plot(NULL,xlim=c(0,3),ylim=c(0,1.05),
             bty="l",xlab=bquote(~phi), ylab="Predictive Power",main=paste0("Z(",tadapt,")=",z()))
        grid(nx=NULL,ny=NULL)


        for(i in 1:numpara){
          lines(temp$data[,1],temp$data[,1+2*numpara+i],col=mycol[i],lwd=2)
          text(2.5, temp$data[nrow(temp$data), 1+2*numpara+i]-0.01, paste0("by t=",newfutimes()[i]))
        }
        mtext(bquote("Spending Function"~alpha*t^phi),side=3)
      })
